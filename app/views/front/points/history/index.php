...
<div class="w-full max-w-[1300px] mx-auto space-y-5 mt-8">
    ...
    <div class="relative bg-ink rounded-[2.5rem] p-8 md:p-10 shadow-lg overflow-hidden flex flex-col xl:flex-row items-center justify-between gap-8 border border-gray-800">
        <!-- افکت درخشان پس‌زمینه (Glow) -->
        <div class="absolute -top-32 -right-32 w-96 h-96 bg-coral-500/20 blur-[100px] rounded-full pointer-events-none"></div>
        <div class="absolute -bottom-32 -left-32 w-80 h-80 bg-blue-500/10 blur-[100px] rounded-full pointer-events-none"></div>
        ...
        <div class="relative z-10 flex flex-wrap justify-center gap-4">
            <!-- امتیاز فعلی -->
            <div class="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-5 text-center min-w-[150px] transition-transform hover:scale-105">
                <span class="block text-gray-400 text-sm font-medium mb-1">امتیاز فعلی</span>
                <span class="block text-2xl md:text-3xl font-bold text-white"><?= number_format($currentPoint->points) ?></span>
            </div>
            <!-- کل کسب‌شده -->
            <div class="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-5 text-center min-w-[150px] transition-transform hover:scale-105 shadow-[0_0_20px_rgba(255,107,107,0.1)]">
                <span class="block text-gray-400 text-sm font-medium mb-1">کل کسب‌ شده</span>
                <span class="block text-2xl md:text-3xl font-bold text-coral-500"><?= number_format($totalEarned) ?></span>
            </div>
            <!-- کل تراکنش‌ها -->
            <div class="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-5 text-center min-w-[150px] transition-transform hover:scale-105">
    ...
    <div class="pt-2 space-y-4">
        ...
        <?php if (!empty($pointLogs)): ?>
            <div class="bg-white border border-border rounded-[2rem] overflow-hidden shadow-sm">
                ...
                <!-- صفحه‌بندی -->
                <div class="px-6 py-4 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4 bg-white rounded-b-[2rem]">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <?php if ($currentPage > 1): ?>
                            <a href="<?= URL_ROOT ?>points/history?page=<?= $currentPage - 1 ?>" class="p-2 rounded-xl border border-border text-gray-600 hover:text-ink hover:bg-gray-100 transition-colors duration-200">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                </svg>
                            </a>
                        <?php else: ?>
                            <button class="p-2 rounded-xl border border-border text-gray-400 disabled:opacity-50 disabled:bg-transparent cursor-not-allowed" disabled>
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                </svg>
                            </button>
                        ...
                        <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
                            <?php if ($i == $currentPage) : ?>
                                <a href="<?= URL_ROOT ?>points/history?page=<?= $i ?>" class="w-9 h-9 rounded-xl bg-coral-500 text-white font-medium flex items-center justify-center shadow-sm text-sm hover:bg-coral-600 transition-colors duration-200">
                                    <?= $i ?>
                                </a>
                            <?php else: ?>
                                <a href="<?= URL_ROOT ?>points/history?page=<?= $i ?>" class="w-9 h-9 rounded-xl bg-white border border-border text-gray-600 hover:bg-gray-100 hover:text-ink font-medium flex items-center justify-center transition-colors duration-200 text-sm">
                                    <?= $i ?>
                                </a>
                            <?php endif ?>
                        <?php endfor ?>
